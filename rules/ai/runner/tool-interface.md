...
- **Subject:** every per-tool adapter. Today: the Claude adapter (see `rules/ai/runner/claude-invocation.md`) and the Codex adapter (see `rules/ai/runner/codex-invocation.md`). Any future adapter falls under this rule the moment it is added.
...
- `model` — the model identifier persisted in `.flanders/config.json` per `rules/flanders-config/file-format.md`. An empty string means "default configured model" and the adapter must not pass an explicit model flag to its binary.
...
- `resumeSessionId` — when set, the adapter resumes the previous session with that id (used by `rules/ai/task-context/worker-continuity.md`). When unset, the adapter starts a fresh invocation.
- `forkParentSessionId` — when set, the adapter forks from that parent session (used by the branch-A case in `rules/ai/task-context/prep-optimization.md`). When unset, no fork happens.
...
A surface-able piece of tool activity that the runner forwards to the UI region defined in `contracts/cli-commands/implement/ui.md`. Field shape:
...
- `retryable` — boolean. `true` means the runner must retry per `rules/ai/retry/retry-on-errors-and-rate-limits.md` and `rules/ai/retry/transient-error-backoff.md`. `false` means the failure propagates to the caller as a non-retryable error.
