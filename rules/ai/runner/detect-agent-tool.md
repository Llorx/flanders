...
The build/test detection agent spawned by `implement` at workspace setup time (see `contracts/cli-commands/implement/workspace.md`) is not separately configured in `.flanders/config.json`. It runs through the AI runner with the same `tool`, `model`, and `effort` the runner uses for the worker.
...
1. The orchestrator reads `.flanders/config.json` per `rules/flanders-config/file-format.md`.
...
3. The runner invokes the resulting tool with that model and effort, per `rules/ai/runner/claude-invocation.md` or `rules/ai/runner/codex-invocation.md`.
