...
- **Subject:** the orchestrator of the `implement` inner loop, at the adversarial review stage, once every configured reviewer has run to a verdict (every per-reviewer error file is present per `rules/ai/agents/reviewer-verdict-via-error-log.md`).
