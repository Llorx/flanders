...
- **Subject:** the orchestrator of the `implement` inner loop, at the adversarial review stage, when it launches the reviewers configured in the `reviewers` array (see `shared/flanders-config.md`).
- **Not subject:** the AI runner, which absorbs the retries and rate-limit waits of the single invocation it is given (see `contracts/cli-commands/implement/ai-runner.md`); it is unaware that sibling reviewer invocations are running alongside it.
...
2. **Independent retry and rate-limit handling.** Each reviewer's runner invocation absorbs that reviewer's own retryable errors and rate-limit waits per `rules/ai/retry/retry-on-errors-and-rate-limits.md`. One reviewer entering a rate-limit wait does not pause, delay, or restart the other reviewers; each proceeds, retries, and waits on its own schedule.

3. **Each writes only its own per-reviewer error file.** Because each reviewer writes exclusively to its own per-reviewer error file (per `rules/ai/agents/reviewer-verdict-via-error-log.md`) and performs no project writes, concurrent execution produces no write contention between reviewers.

4. **The stage waits for the last reviewer.** The stage's reviewer work completes only once every reviewer has run to a verdict (including any per-reviewer re-launches for an absent file). The orchestrator forms the stage verdict (per `rules/ai/agents/review-stage-aggregates-error-logs.md`) only after the last reviewer has finished.
