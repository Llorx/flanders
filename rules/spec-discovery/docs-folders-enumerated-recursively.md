...
- The discovery walks from the project root and, for every directory named `.docs`, collects the files under its `contracts/` subfolder as contracts and the files under its `rules/` subfolder as rules.
...
- A Flanders surface lists only a fixed root `contracts/` and `rules/` pair and misses contracts or rules that live in nested `.docs` folders.
