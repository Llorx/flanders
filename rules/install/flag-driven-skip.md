...
| `--reviewer-tool=<value>` | Reviewer tool |
| `--reviewer-model=<value>` | Reviewer model |
| `--reviewer-effort=<value>` | Reviewer effort |
...
For `--worker-model`, `--worker-effort`, `--reviewer-model`, `--reviewer-effort`, an empty value is a valid answer that resolves to `""` in `.flanders/config.json` per `rules/flanders-config/file-format.md`. An empty value is therefore distinct from "flag not supplied":
...
For `--worker-tool`, `--worker-effort` (when discrete), `--reviewer-tool`, `--skills-tool`, the set of valid values is closed (see the install contract). A supplied value outside the closed set is a usage error pinned by the contract — this rule does not relax that.
