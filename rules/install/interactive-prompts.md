...
Every interactive question `install` asks the user (scope, skills tool, worker tool, worker model, worker effort, reviewer tool, reviewer model, reviewer effort) goes through the same prompt helper that `implement` uses for its plan-selection question. There is one prompt helper for the whole library, and every interactive prompt across every command goes through it.
