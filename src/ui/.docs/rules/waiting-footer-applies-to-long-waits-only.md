...
- A transient-error backoff (see `src/ai/.docs/rules/retry/transient-error-backoff.md`). These waits are capped at one minute and the user does not benefit from a dedicated footer state for them; the footer stays in its normal `Working` state and the working animation continues.
