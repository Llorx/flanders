...
A command that owns a region pinned to the terminal during its run has exactly one output object responsible for that region, which is also the sole channel through which every stdout/stderr write the command produces flows (the writes it covers are enumerated under `What flows through the owner` below).
...
- **Scope:** the transition between the normal footer state and the waiting footer state defined by the UI contract, for every stage in which a single AI agent runs at a time — the worker stage and the build and test command detection stage that runs at startup (see [.spec/contracts/cli-commands/implement/workspace.md](/.spec/contracts/cli-commands/implement/workspace.md)). The normal footer state is the working footer state during those stages (see [.spec/contracts/cli-commands/implement/ui.md](/.spec/contracts/cli-commands/implement/ui.md)). The adversarial review stage is out of this scope: it does not use the global waiting footer state. A reviewer's rate-limit wait during the review stage is surfaced as that reviewer's `waiting` status with its own countdown inside the reviewing footer line (see [.spec/contracts/cli-commands/implement/ui.md](/.spec/contracts/cli-commands/implement/ui.md), `Footer line — reviewing state`), not by switching the whole footer into the global waiting state.
