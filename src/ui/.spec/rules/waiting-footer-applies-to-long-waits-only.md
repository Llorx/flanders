...
- A transient-error backoff (see [src/ai/.spec/rules/retry/transient-error-backoff.md](/src/ai/.spec/rules/retry/transient-error-backoff.md)). These waits are capped at one minute and the user does not benefit from a dedicated footer state for them; the footer stays in its normal footer state — `Preparing` during the prep stage, `Working` during the worker stage — and the animation continues.
