...
function mockFs(initialContent:string):{ fs:FsContext; content():string } {
    ...
    return {
        fs: {
            ...
            stat() { return Promise.resolve({ size: stored.length, isFile: true, isDirectory: false, mtimeMs: 0 }); },
...
test.describe("PlanFile.rename", test => {
    function pathTrackingFs(initialPath:string, initialContent:string) {
        ...
        const fs:FsContext = {
            ...
            readdir() { return Promise.resolve([]); },
            stat(p:string) { return files.has(p) ? Promise.resolve({ size: files.get(p)!.length, isFile: true, isDirectory: false, mtimeMs: 0 }) : Promise.reject(new Error("not found: " + p)); },
