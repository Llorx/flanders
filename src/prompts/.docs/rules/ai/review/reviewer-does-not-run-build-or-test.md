...
4. **Only read-only git commands.** The only commands the reviewer runs are the read-only git operations it uses to derive the change set (see [src/prompts/.docs/rules/ai/review/reviewer-derives-change-set-from-git.md](/src/prompts/.docs/rules/ai/review/reviewer-derives-change-set-from-git.md)).
