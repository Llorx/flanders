...
- A task that constrains how the work is done — reusing existing code unchanged, leaving a function, type, or file untouched, or declaring code out of the task's scope — is achievable against the baseline it leaves immutable: every contract and rule the task's acceptance criteria require its output to satisfy is already satisfied by that immutable baseline, so honoring the no-touch constraint still leaves every acceptance criterion reachable. Before writing such a task, confirm this for each obligation the acceptance criteria impose on the output: check that the code the task leaves untouched already meets it. When the immutable baseline carries a gap against a required obligation that the no-touch constraint would prevent the task from closing, the acceptance criteria are unreachable and the task is invalid even when every claim it makes about the baseline is accurate; instead widen the task's scope — or the producing earlier task's scope — to authorize and ground the change that closes the gap, or escalate the conflict to the user during the clarification phase. This is a feasibility check, not a license to leave the mechanism open: still close every implementation choice to a single concrete value rather than defer it to the worker.
