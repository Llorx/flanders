...
- Every leaf task leaves the project building and its tests passing. The build and test gates run inside every task's own cycle, so each task is cut such that the code it produces compiles and the whole test suite passes at that task's end: the compilation, the type checking, and the test updates a change breaks are delivered by the task that breaks them, and work that only holds together as a unit — a signature change and the call sites it breaks, a behavior change and the tests asserting the previous behavior — lands in one task however large that task becomes.
