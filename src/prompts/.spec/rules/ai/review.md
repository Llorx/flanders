...
2. **The spec is judged against `HEAD`.** The reviewer verifies each element of the spec against the committed working tree — through the build and test gates that already passed before the review, an existing test whose assertion a regression would trip, or its own inspection of the full working tree at `HEAD`, as the element allows. The reviewer does not require the evidence to originate from an uncommitted diff.
