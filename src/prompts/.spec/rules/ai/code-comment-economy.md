...
- **Subject:** the construction of the Flanders prompts through which an agent authors source code in the host project — the worker prompt of the single-task cycle (see [.spec/contracts/shared/task-cycle.md](/.spec/contracts/shared/task-cycle.md)), however the surface running that cycle builds it — at the point where the prompt instructs the agent how to write the code.
