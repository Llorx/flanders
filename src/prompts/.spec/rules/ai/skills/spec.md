...
Additionally, under the per-skill-inputs provision of [src/prompts/.spec/rules/ai/skills/skills-common.md#every-flanders-content-skill-hosts-its-final-validator-the-same-way](/src/prompts/.spec/rules/ai/skills/skills-common.md#every-flanders-content-skill-hosts-its-final-validator-the-same-way), the validator reads the on-disk source that a file written this run and a corpus spec jointly govern, so category C's non-contradiction judgment can be grounded in the behavior that source pins rather than in the two spec texts alone. Reading source is read-only and does not relax the validator's read-only discipline.
...
- The validator reports PASS on a run whose newly written file and an existing corpus spec are each individually coherent but jointly unsatisfiable against the behavior the governed source pins, because it compared only the two spec texts and never read that source.
...
3. **Category C** — the verdict names the corpus files the validator read and compared to reach its non-contradiction judgment; a C verdict that names no consulted corpus file is not an adjudication. A flagged contradiction quotes both sides with their file:line. When the non-contradiction judgment is source-grounded per [src/prompts/.spec/rules/ai/skills/spec.md#the-flanders-spec-validator-audits-each-artifact-by-its-folder-against-the-spec-check-categories](/src/prompts/.spec/rules/ai/skills/spec.md#the-flanders-spec-validator-audits-each-artifact-by-its-folder-against-the-spec-check-categories), the verdict also names the `file:line` of the governed source the validator read, and a flagged source-grounded contradiction cites that source alongside both obligations.
