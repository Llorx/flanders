...
- **Not subject:** a prohibition that is load-bearing per the test below. Such a prohibition is a legitimate obligation, not a violation — for example [src/commands/.spec/rules/ai/agents/no-git-writes.md](/src/commands/.spec/rules/ai/agents/no-git-writes.md) and [src/prompts/.spec/rules/ai/review/reviewer-does-not-run-build-or-test.md](/src/prompts/.spec/rules/ai/review/reviewer-does-not-run-build-or-test.md), which each forbid a behavior an implementer would otherwise plausibly perform.
