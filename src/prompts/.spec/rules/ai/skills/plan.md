...
## `/flanders-plan` tasks are achievable against the immutable baseline they constrain

A task constrains a mechanism when it fixes how the work is carried out rather than only what the work must achieve — reusing an existing helper unchanged, leaving a named function, type, or file untouched, or declaring some code out of the task's scope. `/flanders-plan` admits such a constraint only when the baseline the task leaves immutable already satisfies every contract and rule the task's own acceptance criteria require the task's output to satisfy. The planner confirms this before writing the task: for each obligation the acceptance criteria impose on the output, it checks that the code the task leaves untouched already meets that obligation, so that honoring the no-touch constraint still leaves every acceptance criterion reachable. When the immutable baseline carries a gap against such a required obligation — a behavior a linked contract or rule demands that the baseline does not yet provide — and the no-touch constraint would prevent the task from closing that gap, the acceptance criteria are unreachable and the task is invalid however accurately it describes the code.

This obligation is distinct from [src/prompts/.spec/rules/ai/skills/plan.md#flanders-plan-grounds-every-code-touching-task-in-the-code-it-builds-on](/src/prompts/.spec/rules/ai/skills/plan.md#flanders-plan-grounds-every-code-touching-task-in-the-code-it-builds-on): grounding fails a task that misstates the code it builds on, while this rule fails a task whose every statement about the baseline is accurate yet whose acceptance criteria still cannot be met without breaking the task's own mechanism constraint. A task can satisfy grounding and violate this rule at the same time.

`/flanders-plan` resolves an unreachable task in one of two ways, never by persisting it as written:

- **Widen the scope.** The task — or, when the baseline is produced by an earlier task in the plan, that earlier task — is rewritten to authorize the change to the baseline that closes the gap, grounding and justifying that change per [src/prompts/.spec/rules/ai/skills/plan.md#flanders-plan-grounds-every-code-touching-task-in-the-code-it-builds-on](/src/prompts/.spec/rules/ai/skills/plan.md#flanders-plan-grounds-every-code-touching-task-in-the-code-it-builds-on). A widened earlier task stays ordered before the task that depends on it.
- **Escalate.** When whether the no-touch constraint or the required obligation should give way is a decision the user must make — for example because closing the gap would change behavior another contract or rule pins — the planner raises the conflict during the clarification phase per [src/prompts/.spec/rules/ai/skills/plan.md#flanders-plan-clarification-questions-are-limited-to-genuinely-unsettled-implementation-choices-scope-ambiguities-and-unbacked-runtime-premises](/src/prompts/.spec/rules/ai/skills/plan.md#flanders-plan-clarification-questions-are-limited-to-genuinely-unsettled-implementation-choices-scope-ambiguities-and-unbacked-runtime-premises), rather than silently choosing a side.

This rule adds a feasibility check; it does not relax the requirement that every implementation choice the request leaves open be closed to a single concrete value rather than deferred to the worker. The planner still fixes the mechanism instead of leaving it open. Achievability governs which fixed mechanisms are admissible — only those whose immutable baseline already meets the task's required obligations — not whether the mechanism is fixed at all.

### Who this applies to

- **Subject:** `/flanders-plan` during its drafting phase, on every task that constrains a mechanism — that reuses existing code unchanged, leaves a function, type, or file untouched, or declares code out of the task's scope. The obligation also binds any re-entry of drafting triggered by the post-write fix loop.
...
- A task fixes a mechanism — reuse-unchanged, leave-untouched, or out-of-scope — whose immutable baseline does not satisfy an obligation the task's acceptance criteria require of its output, so the acceptance criteria cannot be met without breaking the constraint.
- A task's acceptance criteria require its output to honor a contract or rule, the code the task leaves untouched does not yet honor that obligation, and the task neither widens its scope to fix the baseline nor escalates the conflict to the user.
- The planner treats accurate claims about the baseline as sufficient and persists a task whose acceptance criteria are unreachable given its own no-touch constraint.
- A task leaves its mechanism open in the name of achievability instead of fixing a concrete mechanism whose baseline already meets the task's required obligations.
...
- **Tasks are achievable against the baseline they leave immutable.** A task that constrains a mechanism — reuses existing code unchanged, leaves a function, type, or file untouched, or declares code out of scope — is FAIL when the immutable baseline does not already satisfy an obligation the task's own acceptance criteria require of its output, so those criteria cannot be met without breaking the constraint. Audit each mechanism-constraining task by reading the baseline it leaves untouched — the current on-disk source, plus the changes any earlier task it depends on prescribes — and confirming it already honors every contract and rule the task's acceptance criteria impose on its output; a gap the no-touch constraint cannot close, neither widened into the task's (or an earlier task's) scope nor escalated to the user, is FAIL. This is distinct from the claims-accuracy check above: a task whose every statement about the baseline is accurate is still FAIL when its acceptance criteria are unreachable given its own constraint. Per [src/prompts/.spec/rules/ai/skills/plan.md#flanders-plan-tasks-are-achievable-against-the-immutable-baseline-they-constrain](/src/prompts/.spec/rules/ai/skills/plan.md#flanders-plan-tasks-are-achievable-against-the-immutable-baseline-they-constrain).
...
- The validator reports PASS on a task that constrains a mechanism — reuse-unchanged, leave-untouched, or out-of-scope — whose immutable baseline does not satisfy an obligation the task's acceptance criteria require of its output, so the criteria cannot be met without breaking the constraint, because the task's claims about the baseline are all accurate.
