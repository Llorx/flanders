...
- **Task granularity is sane.** A leaf task is not so broad that it would need to be split into separate AI invocations, and not so narrow that splitting further would create artificial fragmentation. Granularity is adjudicated with one verdict line per leaf task under [src/prompts/.spec/rules/ai/skills/plan.md#the-flanders-plan-validator-renders-a-granularity-verdict-for-every-leaf-task](/src/prompts/.spec/rules/ai/skills/plan.md#the-flanders-plan-validator-renders-a-granularity-verdict-for-every-leaf-task).
