...
- **Subject:** the regression test that validates the complete set of final assembled Flanders skill artifact bodies that `install` can write.
- **Not subject:** files inside flanders' own `.spec/contracts`, `.spec/rules`, and `.spec/flanders` folders (at any depth in the project tree) and flanders' own `plans/` folder, which freely cross-reference each other by relative path. Those files are flanders' development spec and never ship to user projects.

### What is forbidden in a skill artifact body

- A relative, absolute, or namespace-form path that names a specific markdown file inside one of flanders' own `.spec/contracts`, `.spec/rules`, or `.spec/flanders` folders, or inside flanders' `plans/` folder. Omitting the `.spec/` segment does not make a namespace-form citation valid. Examples of what NOT to embed: `.spec/contracts/ai-skills/spec-skill.md`, `src/prompts/.spec/rules/ai/skills/skills-common.md#every-flanders-content-skill-hosts-its-final-validator-the-same-way`, `rules/ai/agents/no-background-commands.md`, `.spec/flanders/file-placement.md`, `.spec/contracts/shared/spec-folder-write-authority.md`, `plans/2026-01-01_00.00-example.md`.
...
The body never embeds a specific file path that points to a file from flanders' own spec or plans.

### How to apply this rule

When authoring or editing a skill artifact body source, search the body for any path that names a specific markdown file inside a `.spec/contracts`, `.spec/rules`, or `.spec/flanders` folder at any depth, or inside the `plans/` folder, including a namespace-form path that omits the `.spec/` segment. Every such citation is removed. The substantive obligation the citation pointed at is inlined in its place — the citation is the only thing being stripped; the obligation itself stays in the body. The result is a body that, when shipped to an arbitrary user project, makes sense in that project without requiring access to flanders' own repository.
...
A regression test takes the complete set of final assembled Flanders skill artifact body strings that `install` can write, applies a global regular-expression detector for the forbidden paths to every body, collects every match, and asserts that the exact match count is `0`. A boolean presence check is not this guard. The test consumes the assembled body strings as values and never reads a production source file as text.

The detector recognizes markdown-file paths under `.spec/contracts`, `.spec/rules`, `.spec/flanders`, and `plans/` in relative, absolute, and nested forms, including namespace-form paths that omit the `.spec/` segment, with `/` or `\` separators and with or without a trailing markdown fragment. The test exercises the detector with positive fixtures for every governed namespace, nested paths, namespace-form paths that omit `.spec/`, both separator styles, and fragment suffixes, plus negative fixtures for the permitted folder-only structural references. These controls establish that a zero count over the skill bodies means no forbidden path matched, rather than that the detector matches nothing.

### Failure signals

- A skill artifact body, once written by `install` into a user project, names a specific file inside a `.spec/contracts`, `.spec/rules`, or `.spec/flanders` folder, or the `plans/` folder — and that specific file does not exist in the user's project (because it belongs to flanders' own development spec).
...
- The regression guard omits an installable skill body, tests only boolean presence, accepts a non-zero match count, reads production source as text, or lacks detector fixtures that cover the governed path forms and distinguish them from permitted folder-only references.
