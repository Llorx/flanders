...
1. **Does not commit.** It runs no `git add`, `git commit`, or any other git command that mutates repository state. The changes remain as an uncommitted working tree for the user to review, amend, commit, or discard. (The reviewer subagent is separately forbidden from writing to git by [src/commands/.spec/rules/ai/agents/no-git-writes.md](/src/commands/.spec/rules/ai/agents/no-git-writes.md); this rule pins that the skill's own finalization also performs no commit.)
