...
When the enumeration above is empty — `git status --porcelain` reports no files — the verdict the reviewer reaches is pinned by [src/prompts/.spec/rules/ai/review/reviewer-empty-change-set-judged-against-head.md](/src/prompts/.spec/rules/ai/review/reviewer-empty-change-set-judged-against-head.md).

All of these are read-only git operations and are permitted under [src/commands/.spec/rules/ai/agents/no-git-writes.md](/src/commands/.spec/rules/ai/agents/no-git-writes.md); this rule never authorizes the reviewer to mutate repository state.
