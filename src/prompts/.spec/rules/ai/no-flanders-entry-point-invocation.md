# No Flanders entry-point invocation from a spawned agent

## Every Flanders agent prompt bars the agent from invoking a Flanders skill or running a Flanders entry-point command

Every prompt Flanders authors for an agent it spawns tells that agent that Flanders' own entry points are outside its reach: it invokes no `/flanders-…` skill of its AI tool and runs none of the Flanders commands that start a run or rewrite the installation — `implement` (see [.spec/contracts/cli-commands/implement/overview.md](/.spec/contracts/cli-commands/implement/overview.md)), `install` (see [.spec/contracts/cli-commands/install.md](/.spec/contracts/cli-commands/install.md)), and `update` (see [.spec/contracts/cli-commands/update.md](/.spec/contracts/cli-commands/update.md)). The agent already executes inside a Flanders run, so either call re-enters that run, and the prompt states that reason where it states the boundary. The commands that only read stay available to the agent, per [src/prompts/.spec/rules/ai/spec-query-commands-in-prompts.md](/src/prompts/.spec/rules/ai/spec-query-commands-in-prompts.md). When the agent concludes that work one of the entry points owns is needed, it reports that need through the channel its role already reports through.
...
2. **No Flanders entry-point command.** The agent runs no Flanders command that starts a run or rewrites the installation — not directly, not through a package runner, and not through a wrapper or script that reaches one.

3. **The boundary names what stays reachable.** The prompt states that the Flanders commands which only report — the spec-query commands the prompt already names — are outside this boundary and are the agent's to run, so an agent that needs to know which specs govern a path reaches for one instead of reasoning its way around the boundary.

4. **The reason travels with the boundary.** The prompt states why both are barred: the agent already runs inside a Flanders run, so invoking the implementation skill or the implementation command nests a fresh cycle inside the one the agent serves, and an install or update run mid-flight rewrites the skill artifacts that run is executing from.

5. **The need is reported, not acted on.** When the agent concludes that work one of those entry points owns is needed — a spec change, a plan, a further implementation run — it reports that need through the channel its role already reports through and continues with what its role can do, leaving the invocation to the user.
...
- A prompt in scope states the boundary over the Flanders CLI as a whole, so the read-only spec-query commands read as barred and the agent is left inferring applicability instead of asking for it.
