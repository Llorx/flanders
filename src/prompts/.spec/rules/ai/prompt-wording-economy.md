...
- **Subject:** the construction of every prompt Flanders authors as fixed scaffolding — the prompt text Flanders bakes in, as opposed to the per-run content it passes through:
  ...
  - the `/flanders-implement` skill's reviewer prompt;
  - the shared voice and tone instruction text those prompts carry;
  - the `/flanders-spec`, `/flanders-plan`, `/flanders-implement`, and `/flanders-hard-stop-review` skill artifact bodies.
...
- A passage that a shorter wording would convey with the same information, whether it was newly written or edited.
