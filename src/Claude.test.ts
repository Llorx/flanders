...
import { Claude, NonRetryableError } from "./Claude";
...
test.describe("Claude", test => {
    ...
    test("detects rate-limit on full CLI payload (rate_limit_event + assistant + 429 result)", {
        ...
        async ACT({ context, time, claude }) {
            ...
            first.$emitStdout(JSON.stringify({
                type: "assistant",
                message: { model: "<synthetic>", role: "assistant", stop_reason: "stop_sequence", content: [{ type: "text", text: "You've hit your limit · resets 10:40pm (Europe/Madrid)" }] },
                error: "rate_limit",
                session_id: "85843567-dee1-47fd-a1fd-01a47ea4bd27"
            }) + "\n");
            first.$emitStdout(JSON.stringify({
                ...
                result: "You've hit your limit · resets 10:40pm (Europe/Madrid)",
