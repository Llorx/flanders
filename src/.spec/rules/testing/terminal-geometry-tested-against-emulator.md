...
- `arrange-act-assert` remains the only test runner ([src/.spec/rules/testing/library-and-aaa-structure.md](/src/.spec/rules/testing/library-and-aaa-structure.md)); the emulator is a helper used inside the `ARRANGE`/`ASSERT` sections of an `arrange-act-assert` test, never a second runner.
