...
2. The orchestrator invokes the worker through the AI runner with that `session_id` for resumption. The runner translates this to the tool-specific resume invocation (Claude: `--resume <session_id>`; Codex: `codex resume <session_id>` per [src/ai/.spec/rules/runner/claude-invocation.md](/src/ai/.spec/rules/runner/claude-invocation.md) and [src/ai/.spec/rules/runner/codex-invocation.md](/src/ai/.spec/rules/runner/codex-invocation.md)).
