...
In that fresh-invocation branch, the orchestrator does NOT re-inject the task text into the worker prompt, nor regenerate any consolidated reference content. The worker is told what to implement, given the global listing of contract/rule paths, and given the previous-iteration briefing pointing at `error.log`. It re-reads whatever it needs from disk — where the surface consolidated reference content for iteration 1, that file is still in the temporary folder, so the worker can reread it instead of reopening each referenced project file. The cost of re-reading is real but bounded; replaying the full task text and the full reference content on every iteration would burn far more tokens for a marginal gain on the iteration's first turn.
