...
2. **The referenced contract and rule content, consolidated into a single `spec.md` file in the worker's temporary folder** — the orchestrator resolves the markdown links in the task body that point at a file under a `.spec/contracts` or `.spec/rules` folder (per [.spec/contracts/shared/cross-file-reference-links.md](/.spec/contracts/shared/cross-file-reference-links.md)) and writes the referenced content, verbatim, into one `spec.md` file inside the main temporary folder (see [.spec/contracts/cli-commands/implement/workspace.md](/.spec/contracts/cli-commands/implement/workspace.md)); this content is not placed in the prompt. Each distinct referenced file contributes one block, opening with that file's namespace on its own line so the worker can reach the file itself when it wants more than the block carries. When every reference to the file carries a section (heading) anchor that resolves to a heading in it, the block holds the file's preamble followed by each distinct anchored section, preamble and sections alike cut per [.spec/contracts/shared/spec-section-model.md](/.spec/contracts/shared/spec-section-model.md) — a section running from its heading line down to, but not including, the next heading of the same level or of a lower level number, so its nested subsections travel with it. When any reference to the file carries no anchor, or an anchor that resolves to no heading in it, the block holds the whole file, and its separately-anchored sections are not repeated. The worker prompt directs the worker to read `spec.md` in full, from beginning to end, in as few passes as possible — ideally a single read.
3. **The global listing, injected into the prompt** — every contract and rule in the project, each given by its project-root-relative namespace followed by that file's headings (see [.spec/contracts/shared/spec-section-model.md#heading-listing](/.spec/contracts/shared/spec-section-model.md#heading-listing)), so the worker reads what each file carries before deciding to open it and may consult any additional file at its discretion. The listing carries namespaces and headings, in the prompt; the orchestrator writes neither the content of files the task does not reference into `spec.md` nor that content into the prompt.
...
Reading the plan and the referenced files is deterministic work the orchestrator can do in code, at no token cost. Consolidating the referenced obligations into a single `spec.md` the worker reads once — rather than inlining them into the prompt — keeps the prompt itself small while still sparing the worker from locating and opening each referenced file separately, a sequence of reads whose cost grows with every file as the worker's context accumulates. Narrowing by section anchor keeps `spec.md` to the obligations the task actually points at, rather than whole files when only a section applies. The task text stays in the prompt because the worker needs it immediately to know what to build, and the global listing is kept to namespaces and headings because that is what lets the worker judge which unreferenced files bear on its work — a bare namespace names a file without stating what it governs, so a worker given only paths is left guessing from filenames — while staying far cheaper than the content those files hold.
...
- A block in `spec.md` carries content without the namespace of the file it came from, leaving the worker unable to reach that file for the rest of its content.
- A section-narrowed block drops the file's preamble, or ends a section at the first nested subsection heading instead of carrying the nested subsections with it.
...
- The orchestrator omits the global listing, or passes it as bare namespaces without each file's headings, leaving the worker to judge from filenames which files the task did not reference bear on its work.

## The worker resumes its captured session_id across iterations of the same task

For iterations n>1 on the same task, the orchestrator launches the worker by resuming the `session_id` captured during iteration 1. The worker arrives at iteration n>1 knowing what it itself tried in previous iterations — not as a brand-new conversation. The task text and any reference content are not re-injected; the previous-iteration briefing and the worker's continuity from iteration 1 are what carry the necessary context forward.

### Who this applies to

- **Subject:** every orchestrator of the single-task cycle (see [.spec/contracts/shared/task-cycle.md](/.spec/contracts/shared/task-cycle.md)), at the moment it launches an iteration n>1 of the current task — the `implement` command's inner loop and the `/flanders-implement` skill alike.
- **Not subject:** the AI runner, which only knows that the caller asked it to resume a given `session_id`; and the reference-content consolidation of [src/commands/.spec/rules/ai/task-context.md#the-workers-first-iteration-receives-the-task-and-reference-content-by-deterministic-script-injection](/src/commands/.spec/rules/ai/task-context.md#the-workers-first-iteration-receives-the-task-and-reference-content-by-deterministic-script-injection), which is the `implement` command's own.
...
2. The orchestrator launches the worker so that it resumes that `session_id` — the `implement` command through the AI runner, and the `/flanders-implement` skill by re-launching that worker's tool process directly against the same session. Either way the tool receives its own resume invocation: Claude `--resume <session_id>`, Codex `codex exec resume <session_id>` (see [src/ai/.spec/rules/runner.md#the-claude-adapter-spawns-claude---print---output-format-stream-json-and-maps-its-events-to-the-tool-interface](/src/ai/.spec/rules/runner.md#the-claude-adapter-spawns-claude---print---output-format-stream-json-and-maps-its-events-to-the-tool-interface) and [src/ai/.spec/rules/runner.md#the-codex-adapter-spawns-codex-exec---json-and-maps-its-events-to-the-tool-interface](/src/ai/.spec/rules/runner.md#the-codex-adapter-spawns-codex-exec---json-and-maps-its-events-to-the-tool-interface)).
3. **The usage baseline, for a surface that records token metrics.** The `implement` command additionally supplies the worker session's running token totals — the input and output tokens already attributed to that session across its prior iterations — as the resume baseline. The runner passes it to the adapter as `priorSessionUsage` per [src/ai/.spec/rules/runner.md#all-ai-tools-implement-the-same-generic-tool-adapter-interface](/src/ai/.spec/rules/runner.md#all-ai-tools-implement-the-same-generic-tool-adapter-interface), which keeps the task's `it`/`ot` metrics (see [.spec/contracts/shared/plan-file-format.md](/.spec/contracts/shared/plan-file-format.md)) at real consumption when the resumed tool reports session-cumulative usage: the adapter reports only the new iteration's own consumption, never re-counting the tokens of prior iterations. A surface that records no task metrics carries no such baseline.
4. The worker prompt is the standard worker prompt for the iteration plus the previous-iteration briefing defined in [.spec/contracts/shared/task-cycle.md#previous-iteration-error-briefing](/.spec/contracts/shared/task-cycle.md#previous-iteration-error-briefing). The orchestrator does NOT re-inject the task text or any reference content into the prompt for iteration n>1.
5. **Defensive capture.** If, during iteration n>1, the worker returns a `session_id` different from the stored one (renegotiation, regeneration, etc.), the orchestrator updates the stored value for subsequent iterations.

### When resume is not available

When the worker's previous iteration did not yield a capturable `session_id` (none was surfaced, or the call was interrupted before any session id was emitted), iteration n>1 is launched as a **fresh** invocation — no resume.

In that fresh-invocation branch, the orchestrator does NOT re-inject the task text into the worker prompt, nor regenerate any consolidated reference content. The worker is told what to implement, given the global listing of contracts and rules with their headings, and given the previous-iteration briefing pointing at `error.log`. It re-reads whatever it needs from disk — where the surface consolidated reference content for iteration 1, that file is still in the temporary folder, so the worker can reread it instead of reopening each referenced project file. The cost of re-reading is real but bounded; replaying the full task text and the full reference content on every iteration would burn far more tokens for a marginal gain on the iteration's first turn.
...
- A surface that records task metrics resumes the worker without supplying the session's accumulated usage as the resume baseline, so a tool that reports session-cumulative usage makes the task's `it`/`ot` re-count the tokens of prior iterations.
- The orchestrator re-injects the task text or any reference content into the iteration n>1 prompt, replaying material the worker already has access to (via session resume, via the consolidated reference file still in the temporary folder, or via the working-tree files).
...
2. The referenced contract and rule content, consolidated into a `spec.md` file inside that reviewer's own temporary folder — the per-reviewer folder that also holds its `error.log` (see [.spec/contracts/cli-commands/implement/workspace.md](/.spec/contracts/cli-commands/implement/workspace.md)) — resolved from the task body's markdown links per [.spec/contracts/shared/cross-file-reference-links.md](/.spec/contracts/shared/cross-file-reference-links.md), built to the shape the worker's `spec.md` uses (see [src/commands/.spec/rules/ai/task-context.md#the-workers-first-iteration-receives-the-task-and-reference-content-by-deterministic-script-injection](/src/commands/.spec/rules/ai/task-context.md#the-workers-first-iteration-receives-the-task-and-reference-content-by-deterministic-script-injection)). This content is not placed in the prompt. The reviewer prompt directs the reviewer to read its `spec.md` in full, from beginning to end, in as few passes as possible. Each reviewer's `spec.md` lives in that reviewer's own temporary folder, never one shared with another reviewer.
3. The global listing of every contract and rule, in the same namespace-plus-headings shape the worker's listing carries, so the reviewer can consult any additional file at its discretion — which is what lets the reviewer flag a contract or rule that should have applied but was not referenced by the task.
