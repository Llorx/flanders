...
The set of valid values is closed for the single-valued tool flags — `--worker-tool`, `--reviewer-tool`, `--reviewer-N-tool` — and for the effort flags when the tool they apply to is `codex` or `antigravity`: for `codex`, `--worker-effort` / `--reviewer-effort` / `--reviewer-N-effort` validate against Codex's documented effort set; for `antigravity`, the only valid value is empty, because the tool exposes no effort setting, so any non-empty value is a usage error. The `--skills-tool` flag's value is a comma-separated list of one or more distinct names drawn from the closed tool set `claude`, `codex`, `antigravity`. A supplied value outside a closed set is a usage error pinned by the install contract, and this rule does not relax that. By contrast, the model flags (`--worker-model`, `--reviewer-model`, `--reviewer-N-model`) for every tool, and the effort flags when the tool they apply to is `claude`, are open: they accept any value verbatim and are never rejected on value-set grounds (see [.spec/contracts/cli-commands/install.md](/.spec/contracts/cli-commands/install.md)).
...
- **`antigravity`** — the Antigravity CLI exposes no machine-readable models-listing command (its `agy models` listing renders only to an interactive terminal), so `install` does not probe it. Instead, `install` presents a hand-maintained catalog of the models the Antigravity CLI offers for its `--model` selection. Each catalog entry has a human-readable display label and the value persisted when it is chosen; for this catalog the display label and the persisted value are the same string — the model name exactly as the Antigravity CLI lists it, including its trailing reasoning-effort qualifier in parentheses — and the value is persisted verbatim. The catalog is organized into one group per model provider, in the order below.
  ...
  **GPT-OSS**
  ...
  The catalog is a set of suggestions, not a closed set; the user reaches any model the Antigravity CLI accepts but the catalog omits through the custom entry pinned in [src/commands/.spec/rules/install.md#claudes-model-and-effort-lists-include-a-custom-value-entry](/src/commands/.spec/rules/install.md#claudes-model-and-effort-lists-include-a-custom-value-entry). The question is laid out and navigated as a two-tier provider menu by the same mechanics the `claude` model question uses (see [src/commands/.spec/rules/install.md#the-claude-model-question-groups-models-by-family-a-family-entry-at-the-top-each-opening-a-submenu-of-that-familys-models](/src/commands/.spec/rules/install.md#the-claude-model-question-groups-models-by-family-a-family-entry-at-the-top-each-opening-a-submenu-of-that-familys-models)), reading "provider group" for "model family", except that the Antigravity catalog carries no cross-family alias entry and no `[1m context]` variants, and a provider whose catalog lists a single model presents a single-entry submenu. When the Antigravity CLI offers new models, or retires a listed one, this rule is updated to track that change.
...
- For **`antigravity`**, the question is rendered as a provider-grouped two-tier menu: each model provider is a top-level entry that opens a submenu of that provider's catalog entries, plus the synthetic `default configured model` entry, plus the custom entry pinned in [src/commands/.spec/rules/install.md#claudes-model-and-effort-lists-include-a-custom-value-entry](/src/commands/.spec/rules/install.md#claudes-model-and-effort-lists-include-a-custom-value-entry). It follows the same menu mechanics as the `claude` model question (see [src/commands/.spec/rules/install.md#the-claude-model-question-groups-models-by-family-a-family-entry-at-the-top-each-opening-a-submenu-of-that-familys-models](/src/commands/.spec/rules/install.md#the-claude-model-question-groups-models-by-family-a-family-entry-at-the-top-each-opening-a-submenu-of-that-familys-models)), with model providers in place of model families.
...
- **`antigravity`** — the Antigravity CLI exposes no reasoning-effort setting, so it has no effort levels and no effort question is asked; the persisted effort is always the empty `default configured effort`. This set is **closed** and contains only the empty default.
...
- For **`antigravity`**, no effort question is rendered; the persisted effort is always the empty `default configured effort`.
...
- **Subject:** the `install` command, on exactly three questions — the `claude` worker/reviewer model question, the `claude` worker/reviewer effort question, and the `antigravity` worker/reviewer model question.
- **Not subject:** the `codex` model question, the `codex` effort question, and the `antigravity` effort question, which carry no custom entry — the `codex` model list is the authoritative set the probe returns plus `default configured model`, the `codex` effort set is the closed documented set plus `default configured effort`, and the `antigravity` effort question is not asked (see [src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog](/src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog) and [src/commands/.spec/rules/install.md#effort-levels-are-sourced-per-tool-from-each-clis-documented-levels](/src/commands/.spec/rules/install.md#effort-levels-are-sourced-per-tool-from-each-clis-documented-levels)). Any later Flanders command is also not subject: the custom entry exists only at install time.
...
- A `claude` model or effort question, or the `antigravity` model question, is rendered as a plain selectable list with no custom entry, trapping the user in the curated suggestions.
- The custom entry is offered on a `codex` question or the `antigravity` effort question.
...
- **Not subject:** the `codex` model question (a flat probe-sourced list or a free-text fallback), which uses no grouped menu, and the `antigravity` model question, which uses an analogous provider-grouped two-tier menu — navigated by these same mechanics with model providers in place of model families — pinned in [src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog](/src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog), not this `claude` family menu; every effort question, which is unaffected; and any later Flanders command, which consumes the persisted model string opaquely and never re-renders this menu.
...
- The `claude` family grouping is applied to the `codex` model question, which is flat. The `antigravity` model question is grouped by model provider, not by this `claude` family menu; its grouping is pinned in [src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog](/src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog).
