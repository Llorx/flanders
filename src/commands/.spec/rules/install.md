...
The set of valid values is closed for the single-valued tool flags — `--worker-tool`, `--reviewer-tool`, `--reviewer-N-tool`. The `--skills-tool` flag's value is a comma-separated list of one or more distinct names drawn from the closed tool set `claude`, `codex`. A supplied value outside a closed set is a usage error pinned by the install contract, and this rule does not relax that. By contrast, the model flags (`--worker-model`, `--reviewer-model`, `--reviewer-N-model`) and the effort flags (`--worker-effort`, `--reviewer-effort`, `--reviewer-N-effort`), for every tool, are open: they accept any value verbatim and are never rejected on value-set grounds (see [.spec/contracts/cli-commands/install.md](/.spec/contracts/cli-commands/install.md)).
...
- **`codex`** — `install` queries the tool's CLI for the model list. The probe is a specific subprocess invocation that runs at most once per tool per `install` run, even if the same tool is asked about twice (worker and reviewer); the result is cached and reused across every model and effort question for that tool, so the effort questions add no further subprocess (the effort question's use of this payload is pinned in [src/commands/.spec/rules/install.md#effort-levels-are-sourced-per-tool-codex-per-model-from-the-same-probe-claude-from-a-hand-maintained-catalog](/src/commands/.spec/rules/install.md#effort-levels-are-sourced-per-tool-codex-per-model-from-the-same-probe-claude-from-a-hand-maintained-catalog)). The invocation is `codex debug models` (no `--bundled`, so the catalog is refreshed for the user's account rather than read from the binary's bundled snapshot). The probe captures the launched process's stdout and stderr rather than streaming them to the user's terminal.

  When the launched process completes, the probe **first** tries to interpret the captured stdout as the model catalog, before it consults the exit code or any command-not-found signal: it parses stdout as JSON and checks that it has the shape `{"models":[{"slug":"…","visibility":"…"}, …]}` — an object whose `models` is an array of entries, each carrying a string `slug` and a string `visibility`, and optionally a `supported_reasoning_levels` array whose entries each carry a string `effort` — the per-model effort source the effort question consumes (see [src/commands/.spec/rules/install.md#effort-levels-are-sourced-per-tool-codex-per-model-from-the-same-probe-claude-from-a-hand-maintained-catalog](/src/commands/.spec/rules/install.md#effort-levels-are-sourced-per-tool-codex-per-model-from-the-same-probe-claude-from-a-hand-maintained-catalog)); its presence is not part of this shape gate, so an entry lacking it is still a valid model entry. When stdout is a catalog of that shape, the probe accepts it on its own terms, regardless of the exit code: the list is the `slug` of every entry whose `visibility` is `"list"`, in catalog order; entries with any other `visibility` (for example `"hide"`, used for internal models) are excluded, mirroring Codex's own `/model` picker. A catalog of that shape with at least one `"list"`-visible entry yields the list; a catalog of that shape with no `"list"`-visible entry means `codex` ran and exposed an empty catalog, and `install` falls back to free-text silently. Because a catalog is accepted on the strength of its shape, a `"list"`-visible model is never reinterpreted as a start failure when not-found-like prose appears elsewhere inside the payload (for example inside instruction or description strings the JSON carries).
...
## Effort levels are sourced per tool: codex per-model from the same probe, claude from a hand-maintained catalog

When `install` asks the user for a reasoning-effort level for the worker or the reviewer, the effort levels are sourced per tool. For `codex`, they come from the same account query that supplies the `codex` model list (see [src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog](/src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog)): each model entry the probe returns carries the effort levels that model supports, and the effort question offers the levels of the model selected in the preceding model question. For `claude`, whose CLI exposes no query for its effort levels, `install` presents a hand-maintained catalog of the levels Claude Code documents. The contract [.spec/contracts/cli-commands/install.md](/.spec/contracts/cli-commands/install.md) pins the user-visible shape of each rendering; this rule pins, per tool, where the levels come from, whether the set is open, and how each path persists its value.
...
- **`codex`** — the effort levels are the `supported_reasoning_levels` the probe payload reports for the model selected in the preceding model question: each entry's `effort` string, in payload order. Because the levels are the ones the selected model actually supports, they are filtered by model — a model that supports `xhigh`, `max`, or `ultra` offers them and one that does not omits them. This set is **open**: whatever level the user selects or types is accepted verbatim, and the empty "default configured effort" is always available. When the preceding model question did not resolve to a specific reported model — the `default configured model` entry was chosen, a model was typed into the free-text fallback, or `codex` could not be contacted (see the fallbacks in [src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog](/src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog)) — no per-model level set is known and the effort question falls back to free-text entry.
...
- For **`codex`**, when a specific reported model was selected, the question is rendered as a selectable list whose entries are that model's reported effort levels above, plus one synthetic entry at the end labelled `default configured effort`. When no specific reported model was selected (per the fallback above), the question is rendered as a free-text input with the placeholder `leave empty for the default configured effort`.
...
- For `codex`, `install` presents a hardcoded or model-independent effort set instead of the effort levels the probe payload reports for the selected model; or, for `claude`, ships an effort set that diverges from what Claude Code documents.
- `install` runs a second subprocess to discover `codex` effort levels instead of reusing the cached model-probe payload.
- For `codex`, `install` rejects a typed or flag-supplied effort value as a usage error instead of accepting it verbatim.
- When no specific reported `codex` model was selected, `install` guesses a per-model effort list instead of falling back to free-text entry.
...
- **Not subject:** the `codex` model question and the `codex` effort question, which carry no custom entry — the `codex` model list is the authoritative set the probe returns plus `default configured model`, and the `codex` effort question offers the effort levels the probe payload reports for the selected model plus `default configured effort`, or a free-text input when no specific model was selected (see [src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog](/src/commands/.spec/rules/install.md#the-model-list-is-sourced-per-tool-codex-by-probe-claude-from-a-hand-maintained-catalog) and [src/commands/.spec/rules/install.md#effort-levels-are-sourced-per-tool-codex-per-model-from-the-same-probe-claude-from-a-hand-maintained-catalog](/src/commands/.spec/rules/install.md#effort-levels-are-sourced-per-tool-codex-per-model-from-the-same-probe-claude-from-a-hand-maintained-catalog)). Any later Flanders command is also not subject: the custom entry exists only at install time.
...
- **Worker effort, each reviewer effort** — the default entry is the entry whose persisted value equals the stored `effort` (the synthetic `default configured effort` entry when the stored effort is `""`). For `claude`, an effort that is not among the curated suggestions defaults to the custom entry, whose free-text default is set to the stored value. For `codex`, when the effort question renders as a selectable list for the selected model, the default entry is the reported level equal to the stored effort, or the `default configured effort` entry when the stored effort is `""`; a stored `codex` effort the selected model no longer reports is not among the offered entries and that question is answered actively. When the `codex` effort question falls back to free-text, its free-text default is the stored effort.
