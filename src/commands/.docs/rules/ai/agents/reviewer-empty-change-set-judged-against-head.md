...
- **Subject:** the adversarial reviewer agent of the `implement` inner loop, at the moment it decides its verdict, but only when the worker's change set enumerated per `src/commands/.docs/rules/ai/agents/reviewer-enumerates-worker-changes-via-git.md` is empty.
...
2. **Acceptance criteria are judged against `HEAD`.** The reviewer verifies each acceptance criterion against the committed working tree, drawing the evidence each criterion's classification requires per `src/commands/.docs/rules/ai/agents/evidence/claim-evidence-classification.md`: for a toolchain-guarded criterion, the automated signal the project already runs; for a test-guarded criterion, an existing passing test whose assertion a regression would trip; for a review-adjudicated criterion, the reviewer's inspection of the full working tree at `HEAD`. The reviewer does not require the evidence to originate from an uncommitted diff.

3. **The verdict follows from the criteria, not from the diff's size.** The reviewer passes the task — creating its per-reviewer error file empty per `src/commands/.docs/rules/ai/agents/reviewer-verdict-via-error-log.md` — when every acceptance criterion is satisfied at `HEAD`. It records a violation only for an acceptance criterion, contract, or rule that is genuinely unsatisfied at `HEAD`.
