...
- **Subject:** the adversarial reviewer agent of the `implement` inner loop, at the moment it determines which files the worker changed. The project is always a git repository per `.docs/contracts/cli-commands/implement/git-integration.md`, so this enumeration is unconditional.
...
When the enumeration above is empty — `git status --porcelain` reports no files — the verdict the reviewer reaches is pinned by `src/commands/.docs/rules/ai/agents/reviewer-empty-change-set-judged-against-head.md`.
