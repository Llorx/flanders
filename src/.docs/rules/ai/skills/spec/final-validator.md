...
The host follows `src/.docs/rules/ai/skills/final-validator-host.md` for the shared inputs (artifact paths, canonical listings, output spec, read-only discipline, FAIL loop). On top of those, the host MUST inline the verbatim text of every check category below in the validator's prompt.
...
When this run renamed, relocated, or removed a term that can recur across the corpus (per `src/.docs/rules/ai/skills/spec/rename-triggers-corpus-wide-sweep.md`), the host also passes the explicit list of those old term(s). The list is empty when the run changed no such term.
...
- The host packages the validator prompt without inlining the verbatim text of the content-rule categories (A2 and B2), forcing the validator to discover the content obligations by transitive contract reading — which defeats the explicit-categories obligation pinned in `src/.docs/rules/ai/skills/final-validator-host.md`.
