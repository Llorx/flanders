...
The reference set is the existing spec content the new deliverable must be consistent with — every contract under a `.docs/contracts` folder and every rule under a `.docs/rules` folder, discovered across the whole project tree per `src/.docs/rules/spec-discovery/docs-folders-enumerated-recursively.md`:
