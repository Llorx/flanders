...
Cancellation is per-invocation: a caller may cancel an individual in-flight invocation on its own — for example, `implement` cancelling an optional reviewer that is still in a usage-limit wait once its review round can complete without it (see [src/commands/.spec/rules/ai/agents/review-round-completion-condition.md](/src/commands/.spec/rules/ai/agents/review-round-completion-condition.md)). A cancelled invocation aborts its wait promptly and ends without producing a result — neither a successful result nor a non-retryable error — and the runner performs no further retry for it.
