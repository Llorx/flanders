...
The reuse applies exclusively to the retry of the **same turn** after a retryable terminal event — `{ type: "error", retryable: true }` or `{ type: "rate_limit", waitUntilMs }` as defined in [src/ai/.spec/rules/runner/tool-interface.md](/src/ai/.spec/rules/runner/tool-interface.md). In that flow:
