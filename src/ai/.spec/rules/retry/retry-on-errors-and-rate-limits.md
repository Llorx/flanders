...
The AI runner (see [src/ai/.spec/contracts/ai-runner.md](/src/ai/.spec/contracts/ai-runner.md)) decides whether to retry a failed invocation by reading the terminal event the per-tool adapter emits on the generic tool-adapter interface ([src/ai/.spec/rules/runner/tool-interface.md](/src/ai/.spec/rules/runner/tool-interface.md)). The detection logic does not live in the runner — the adapter has already classified the failure by the time it emits an `error` or a `rate_limit` event. The runner only needs to read the event type.
...
- `{ type: "rate_limit", waitUntilMs }` — the runner waits until `waitUntilMs` per [src/ai/.spec/rules/retry/long-wait-chunked-timer.md](/src/ai/.spec/rules/retry/long-wait-chunked-timer.md) (chunked when the wait exceeds an hour), then re-invokes the same adapter with the same arguments, reusing the captured `session_id` per [src/ai/.spec/rules/retry/retry-reuses-session.md](/src/ai/.spec/rules/retry/retry-reuses-session.md).
- `{ type: "error", retryable: true, message }` — the runner waits per [src/ai/.spec/rules/retry/transient-error-backoff.md](/src/ai/.spec/rules/retry/transient-error-backoff.md) (exponential backoff capped at one minute), then re-invokes the same adapter with the same arguments, reusing the captured `session_id`.
...
- For Claude: [src/ai/.spec/rules/runner/claude-invocation.md](/src/ai/.spec/rules/runner/claude-invocation.md) (sections on event mapping and on error/rate-limit emission).
- For Codex: [src/ai/.spec/rules/runner/codex-invocation.md](/src/ai/.spec/rules/runner/codex-invocation.md) (same sections).
