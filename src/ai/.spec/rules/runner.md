...
There are exactly five `ToolEvent` variants. Adapters must not invent additional variants; the runner must not handle anything outside this set. Token usage is reported through the `onUsage` callback described above, not by adding a sixth event type or by attaching payload to `done`. An authentication/login failure is not a sixth variant either: it is carried by the `fatal` field of the `error` variant below.
...
#### `{ type: "error", retryable, fatal, message }`
...
- `fatal` — boolean, defaulting to `false`. `true` marks an authentication/login failure — the tool cannot run at all until the user re-authenticates. An adapter sets `fatal: true` only for an authentication/login failure, and always pairs it with `retryable: false`, because such a failure never clears by waiting and re-invoking. The runner never retries a `fatal` error and surfaces it to the caller as a run-ending fatal failure distinct from an ordinary non-retryable error, per [src/ai/.spec/rules/retry.md#the-runner-retries-retryable-errors-and-rate-limits-via-the-tool-interface-events](/src/ai/.spec/rules/retry.md#the-runner-retries-retryable-errors-and-rate-limits-via-the-tool-interface-events) and [src/ai/.spec/contracts/ai-runner.md](/src/ai/.spec/contracts/ai-runner.md). Every non-authentication error carries `fatal: false`.
- `message` — short, human-readable reason. Used for surfacing to the user (UI region) and for logging to `error.log`. The runner never inspects `message` to decide retryability or fatality — the `retryable` and `fatal` booleans are authoritative.
...
The decision of whether a given failure is retryable — and whether it is a fatal authentication/login failure — lives entirely in the adapter. The runner does not second-guess it. Adapters classify their tool's native error surface into retryable / non-retryable / fatal-authentication according to the rule that pins each tool's adapter (see [src/ai/.spec/rules/runner.md#the-claude-adapter-spawns-claude---print---output-format-stream-json-and-maps-its-events-to-the-tool-interface](/src/ai/.spec/rules/runner.md#the-claude-adapter-spawns-claude---print---output-format-stream-json-and-maps-its-events-to-the-tool-interface), [src/ai/.spec/rules/runner.md#the-codex-adapter-spawns-codex-exec---json-and-maps-its-events-to-the-tool-interface](/src/ai/.spec/rules/runner.md#the-codex-adapter-spawns-codex-exec---json-and-maps-its-events-to-the-tool-interface)).
...
- `waitUntilMs` — Unix timestamp in milliseconds: the expected end of the wait, the instant by which the invocation is expected to be able to run again. It is the tool's authoritative reset instant when the tool's signal carries one; otherwise — a rate-limit or quota/credit exhaustion with no end time, or a transient the adapter paces with a fixed delay — it is a value the adapter synthesizes, as pinned by that adapter's own rule. Either way the runner takes it as the wait's expected end and works toward it on the bounded retry cadence of [src/ai/.spec/rules/retry.md#rate-limit-waits-retry-the-invocation-at-most-every-30-minutes](/src/ai/.spec/rules/retry.md#rate-limit-waits-retry-the-invocation-at-most-every-30-minutes), re-invoking the same adapter with the same arguments on each attempt and re-anchoring the expected end whenever a later event reports a different one.
...
- An adapter emits an authentication/login failure as an ordinary `error` with `fatal: false` (retryable or non-retryable), so the runner either loops on it indefinitely or treats it as an ordinary task/reviewer failure instead of ending the run.
- An adapter sets `fatal: true` together with `retryable: true`, or sets `fatal: true` for a failure that is not an authentication/login failure.
...
Claude's `stream-json` output is a sequence of newline-delimited JSON objects. The adapter parses each line and routes the object based on its top-level `type`. The relevant event types and their mapping are listed below; every native event Claude emits is mapped, retained, or filtered as documented here. A `rate_limit_event` is a relevant native event: Claude emits it to report where the invocation stands against its usage limits, whatever that standing is, so the event reports ordinary utilization as readily as a rejection. The adapter retains the `rate_limit_info` of the most recent such event as a source of the limit's reset instant, and consults it only once the terminal `result` has established that the failure is a rate-limit.
...
**When `result.is_error === true`** — the terminal `result` establishes what the failure is. The adapter first checks that event for a rate-limit; absent one, it checks it for an authentication/login failure; absent that, it consults its `api_error_status` and `subtype`:

- **A rate-limit** — when `api_error_status === 429`, or the `result` carries a `rate_limit_info` whose status marks the request as rejected — the overage status when the request is on overage, otherwise the standard status, equal to `rejected` — the failure is a rate-limit, and the adapter emits a `rate_limit` event. Every other status value the info object can carry reports a standing the invocation is allowed to keep running under, so the adapter leaves that failure to the `api_error_status` ladder below. `waitUntilMs` is the parseable reset timestamp of the `result`'s own `rate_limit_info` when it carries one — the overage reset when the request is on overage, otherwise the standard reset — and the reset of the retained `rate_limit_event.rate_limit_info` when the `result` carries none. When neither source yields a parseable reset — a bare 429 with no info object anywhere in the invocation, or info objects carrying no reset — the adapter synthesizes the wait, the same 8-to-12-minute window the Codex adapter uses (see [src/ai/.spec/rules/runner.md#the-codex-adapter-spawns-codex-exec---json-and-maps-its-events-to-the-tool-interface](/src/ai/.spec/rules/runner.md#the-codex-adapter-spawns-codex-exec---json-and-maps-its-events-to-the-tool-interface)): emit `{ type: "rate_limit", waitUntilMs: <now> + R }`, where `R` is drawn uniformly at random from the closed interval of 8 minutes to 12 minutes, both the current time and the random draw obtained through the injected contexts per [src/.spec/rules/external-access-through-contexts.md](/src/.spec/rules/external-access-through-contexts.md), never by calling `Date.now()` or `Math.random()` directly. Either way the failure yields a `rate_limit` event, so the runner works the invocation toward the limit clearing and the footer surfaces the rate-limit countdown rather than holding the working state.
- **An authentication/login failure** — when the terminal `result` carries Claude's authentication-failure identifier (its `error` field equal to `authentication_failed`) or `api_error_status === 401`, the failure is a login failure, and the adapter emits `{ type: "error", retryable: false, fatal: true, message: <result.error.message> }`. This branch is checked before the `api_error_status` ladder below, so a login failure is never misrouted into the retryable transport bucket (`api_error_status === null`) or the non-retryable catch-all; that misrouting is exactly what makes the runner retry the failure forever or spend iterations against a wall the user must clear by logging in. The adapter keys this decision off the structured `error` identifier and `api_error_status`, never off the human-readable `result.error.message` text.
...
The adapter never inspects `stderr` or the prompt text to decide retryability; Claude's structured event stream is authoritative, with the terminal `result` establishing what the failure is and the retained `rate_limit_event.rate_limit_info` supplying the reset instant of a failure that `result` has already established to be a rate-limit.
...
- The adapter filters or discards a Claude `rate_limit_event` carrying `rate_limit_info`, so a rate-limit whose terminal `result` carries no reset of its own falls back to a synthesized wait instead of the authoritative reset the stream already exposed.
- The adapter treats a `rate_limit_info` object as a rate-limit on the strength of its presence rather than of its status, so a `rate_limit_event` reporting utilization — a standing the invocation is allowed to keep running under — turns an unrelated failure into a rate-limit wait.
- A retained `rate_limit_event.rate_limit_info` decides the classification of a terminal `result` that establishes a different failure, so a 5xx, a 408/425, or a transport failure with no HTTP response is emitted as a `rate_limit` event and works toward a reset instant instead of taking the transient backoff of [src/ai/.spec/rules/retry.md#transient-retries-use-exponential-backoff-capped-at-one-minute](/src/ai/.spec/rules/retry.md#transient-retries-use-exponential-backoff-capped-at-one-minute).
- On a rate-limit carrying a parseable reset timestamp, the adapter synthesizes an estimated wait instead of using that authoritative window-end.
- On a rate-limit with no parseable reset — including a bare 429 with no `rate_limit_info` object — the adapter falls back to a retryable `error` instead of synthesizing the rate-limit wait; or it synthesizes a wait that falls outside the 8-to-12-minute interval, or computed by calling `Date.now()` / `Math.random()` directly instead of through the injected contexts.
- On an authentication/login failure (`result.error === "authentication_failed"` or `api_error_status === 401`), the adapter emits a retryable `error` (so the runner loops forever) or a non-retryable `error` with `fatal: false` (so the run does not hard-stop), instead of `{ type: "error", retryable: false, fatal: true }`.
...
**Classification of a failure message** — the adapter inspects the trimmed message by literal substring search, case-insensitive for every family below except the reconnect family, whose substring matches case-sensitively. The families are evaluated top to bottom in the order listed below:
...
- Message contains any of: `not logged in`, `codex login`, `not authenticated`, `unauthorized`, or the standalone token `401` — this is an authentication/login failure. Codex cannot run until the user re-authenticates, so the adapter emits `{ type: "error", retryable: false, fatal: true, message }` — the same fatal login failure the Claude adapter emits (see [src/ai/.spec/rules/runner.md#the-claude-adapter-spawns-claude---print---output-format-stream-json-and-maps-its-events-to-the-tool-interface](/src/ai/.spec/rules/runner.md#the-claude-adapter-spawns-claude---print---output-format-stream-json-and-maps-its-events-to-the-tool-interface)). This family is a closed set on the same footing as the other Codex substring families: adding a substring requires updating this rule first. It is checked ahead of the rate-limit family so a login failure is never absorbed as a rate-limit wait.
...
- The adapter classifies a login/authentication message (for example `not logged in`, `codex login`, or `unauthorized`) as a retryable `error`, a rate-limit wait, or a non-retryable `error` with `fatal: false`, instead of `{ type: "error", retryable: false, fatal: true }`.
