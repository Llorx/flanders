...
**When `result.is_error === true`** — the adapter first checks for a rate-limit signal; absent one, it checks for an authentication/login failure; absent that, it consults `api_error_status` and `subtype`:
