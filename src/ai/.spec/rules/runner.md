...
**When `result.is_error === true`** — the terminal `result` establishes what the failure is. The adapter checks that event for an authentication/login failure, then for a rate-limit, and otherwise consults its `api_error_status` and `subtype`:
