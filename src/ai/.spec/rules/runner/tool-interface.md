...
- **Subject:** every per-tool adapter. Today: the Claude adapter (see [src/ai/.spec/rules/runner/claude-invocation.md](/src/ai/.spec/rules/runner/claude-invocation.md)) and the Codex adapter (see [src/ai/.spec/rules/runner/codex-invocation.md](/src/ai/.spec/rules/runner/codex-invocation.md)). Any future adapter falls under this rule the moment it is added.
...
- `resumeSessionId` — when set, the adapter resumes the previous session with that id (used by [src/commands/.spec/rules/ai/task-context/worker-continuity.md](/src/commands/.spec/rules/ai/task-context/worker-continuity.md)). When unset, the adapter starts a fresh invocation.
- `forkParentSessionId` — when set, the adapter forks from that parent session (used by the branch-A case in [src/commands/.spec/rules/ai/task-context/prep-optimization.md](/src/commands/.spec/rules/ai/task-context/prep-optimization.md)). When unset, no fork happens.
...
The adapter has observed the tool's `session_id` for this invocation and surfaces it to the runner. The runner captures it for later reuse per [src/ai/.spec/rules/retry/retry-reuses-session.md](/src/ai/.spec/rules/retry/retry-reuses-session.md) (within-call retry continuity) and per [src/commands/.spec/rules/ai/task-context/worker-continuity.md](/src/commands/.spec/rules/ai/task-context/worker-continuity.md) (cross-iteration worker continuity). Field shape:
...
- `retryable` — boolean. `true` means the runner must retry per [src/ai/.spec/rules/retry/retry-on-errors-and-rate-limits.md](/src/ai/.spec/rules/retry/retry-on-errors-and-rate-limits.md) and [src/ai/.spec/rules/retry/transient-error-backoff.md](/src/ai/.spec/rules/retry/transient-error-backoff.md). `false` means the failure propagates to the caller as a non-retryable error.
