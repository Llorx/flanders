...
- Three **Claude Code skills**, invoked from inside a Claude Code session. Each skill owns its own contract under `ai-skills/`:
  - `/flanders-contract` — see `ai-skills/contract.md`.
  - `/flanders-plan` — see `ai-skills/plan.md`.
  - `/flanders-rule` — see `ai-skills/rule.md`.

The skills are delivered to the user's Claude Code environment by the `install` subcommand.
