...
## Plan content rules
- No task the skill writes may describe work that creates, modifies, deletes, or renames files inside any `.docs/contracts` folder, any `.docs/rules` folder, or the `plans/` folder (with the bounded checkbox/metrics exception that the `implement` command holds, not the worker). The skill's own write boundary and the immovability that applies to the tasks it generates are pinned in `shared/spec-folder-write-authority.md`.
