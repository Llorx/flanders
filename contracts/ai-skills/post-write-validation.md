...
2. **Issues that close a previously-unresolved clarification-scope ambiguity** — i.e., issues whose fix would commit the skill to an answer that, per the originating skill's clarification phase, the user is the one who must give and that the user did not give yet — send the skill back into the clarification phase before any rewrite happens. Re-entered clarification follows the same question mechanics the originating skill's contract already pins: one question per turn, multiple-choice preferred when the answer space is bounded, no bundling. Once the user has answered, the skill applies the fix incorporating that answer.
