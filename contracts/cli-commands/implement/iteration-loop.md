...
5. **Adversarial review stage.** The Flanders configuration holds an ordered list of one or more reviewers (see `shared/flanders-config.md`), each with its own AI tool, model, and effort that may be the same as or different from the worker's and from one another's. The orchestrator runs all configured reviewers for this iteration and only then forms a single verdict from their combined output.
   ...
   A reviewer invocation that ends in an error rather than a successful completion is governed by the AI runner's retry policy (see `rules/ai/retry/retry-on-errors-and-rate-limits.md`); the per-reviewer inspection above is reached only on a successful completion, so a reviewer that fails before writing cannot be mistaken for a passing review.

   Once every reviewer has run to a verdict (every per-reviewer error file is present), the orchestrator forms the stage verdict on a single linear path, with no per-file presence branching: it reads every per-reviewer error file, concatenates their contents in reviewer order with one newline between files, and trims surrounding whitespace and blank lines from the concatenation. See `rules/ai/agents/review-stage-aggregates-error-logs.md`.
...
6. **Commit/check stage.** When the review passed — the adversarial review stage found `error.log` empty after trimming — the task is finalized in this order: first, the task's checkbox is flipped from `[ ]` to `[x]` in the plan file and its metrics object is finalized for that task. Then the orchestrator runs `git add -A` followed by `git commit` with the task's plan number and title verbatim as the commit message (see `cli-commands/implement/git-integration.md`). The inner loop ends with success only after both the plan-file update and the commit have completed.

   When `git commit` exits non-zero, this stage is treated as a failing stage: the checkbox flip is reverted in the plan file, the git output is captured into `error.log`, and the inner loop restarts. The details of this recovery live in `cli-commands/implement/git-integration.md`.
