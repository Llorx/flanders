...
5. **Adversarial review stage.** The Flanders configuration holds an ordered list of one or more reviewers (see `shared/flanders-config.md`), each with its own AI tool, model, and effort that may be the same as or different from the worker's and from one another's. The orchestrator runs all configured reviewers for this iteration and only then forms a single verdict from their combined output.
   ...
   Each reviewer is assigned its own per-reviewer error file inside the temporary folder, named by the reviewer's 1-based position in the configured list: `error.1.log`, `error.2.log`, and so on. Before launching the reviewers, the orchestrator deletes every per-reviewer error file, so none of them exist when the reviewers start. The orchestrator then launches all reviewers concurrently — they are read-only on the project, so running them in parallel is safe — each as its own AI-runner invocation. Every reviewer invocation for the task is a fresh invocation with no continuity from any prior reviewer invocation and no continuity between the reviewers of the same iteration. Each reviewer's own retries and rate-limit waits are absorbed independently by its runner invocation (see `cli-commands/implement/ai-runner.md`). The stage's reviewer work is complete only when the last reviewer has finished.
...
`error.log` holds only the most recent failing iteration's context: the build, test, and commit stages overwrite it with their captured output, and the adversarial review stage overwrites it with the aggregated reviewer violations when the review fails — leaving it untouched when the review passes. The reviewers themselves write to their own per-reviewer error files (`error.1.log`, `error.2.log`, …), never to `error.log`; the orchestrator is what writes the aggregated violations into `error.log` on a failed review. The same fixed `error.log` file name is used regardless of which stage produced the failure (build, test, review, or commit), so the briefing wording stays generic.
