...
No other skill, command, agent, or automated process in this project — including every AI agent that `implement` spawns (prep, worker, reviewer, build/test detect) — may create, modify, delete, or rename any file inside `contracts/`, `rules/`, or `plans/`. A change happens only when the user explicitly invokes the corresponding skill (or, for plans, when `implement` performs its bounded checkbox/metrics update).
