...
While an AI session is waiting on a rate limit, give `F5` a tap and Flanders immediately retries every session waiting at that moment — including concurrent reviewers, neighborino. Working sessions keep working, and the press is not saved for sessions that start later. The key never blocks the run or turns it into a prompt; when input is piped, redirected, or closed, `F5` is simply unavailable and the run carries on unchanged.
