...
## Purpose
Pin who is authorized to create, modify, or delete files inside the project's spec folders: every `.docs/contracts` and `.docs/rules` folder in the project tree, and the project-root `plans/` folder. These hold the project's source of truth — public obligations across each scope's boundary (`.docs/contracts`), implementation conventions internal to each scope (`.docs/rules`), and the ordered work the implementer must execute (`plans/`). The `.docs` layout and scope semantics are pinned in [.docs/contracts/shared/spec-folder-layout.md](/.docs/contracts/shared/spec-folder-layout.md). Authority to change these folders is restricted to dedicated, user-initiated entry points.

## Authority
- Every `.docs/contracts` folder is writable only via the `/flanders-spec` skill (see [.docs/contracts/ai-skills/spec-skill.md](/.docs/contracts/ai-skills/spec-skill.md)).
- Every `.docs/rules` folder is writable only via the `/flanders-spec` skill (see [.docs/contracts/ai-skills/spec-skill.md](/.docs/contracts/ai-skills/spec-skill.md)).
