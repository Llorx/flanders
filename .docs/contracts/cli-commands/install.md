...
- For `claude`, the model question is a hierarchical selection rather than a single flat list. The top level offers a quick entry for the latest model of each family — and for its 1M-context variant where the family offers one — plus the cross-family aliases Claude Code documents, a `pick a specific version` entry that opens a drill-down — first a family, then a concrete version of that family, including its 1M-context variant where the model offers one — the `default configured model` entry, and a final custom entry that opens a free-text input accepting any model identifier the user types. The offered models are a set of suggestions, not a closed set: through the custom entry the user reaches any model Claude Code accepts but the suggestions omit.
