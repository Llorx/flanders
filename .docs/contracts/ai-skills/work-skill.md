...
2. **Build and test.** Once the work is done, the session determines the project's build and test commands and runs them as the validation gates defined in `.docs/contracts/shared/build-test-validation.md` — the same determination and gate semantics the `implement` command uses, applied directly in the session rather than through the `implement` command's temporary scripts. When a gate fails, the session reworks the implementation using that command's output and runs the gates again, repeating until the build and test gates both pass; a gate whose command cannot be determined is skipped. The build and test gates must pass before the review runs.
