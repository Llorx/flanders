...
All of these temporary folders — the main folder together with every per-reviewer folder — are removed automatically when the program exits in any way other than a hard stop. A hard stop is triggered by exceeding the per-task iteration cap ([.spec/contracts/cli-commands/implement/iteration-loop.md](/.spec/contracts/cli-commands/implement/iteration-loop.md)); on a hard stop all of these folders are intentionally preserved on disk so the user can inspect the per-iteration logs, every reviewer's `error.log`, and the briefing `error.log`. If the host platform offers a built-in cleanup-on-exit API, that API is used for the automatic-removal cases; otherwise the program registers an explicit cleanup hook that runs on normal exit and on common termination signals. The hard stop suppresses this cleanup regardless of which mechanism is in use.
