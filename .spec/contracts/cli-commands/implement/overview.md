...
- Hard stop — printed and exit non-zero on a hard stop: when the per-task iteration cap is exceeded for any single task, or when the worker declares the task structurally impossible via `hard-stop.log` (see [.spec/contracts/cli-commands/implement/iteration-loop.md#hard-stop](/.spec/contracts/cli-commands/implement/iteration-loop.md#hard-stop)).
