...
- Hard stop — printed and exit non-zero on a hard stop: when the per-task iteration cap is exceeded for any single task, or when the prep optimization runs and fails irrecoverably (see [src/commands/.spec/rules/ai/task-context/prep-optimization.md](/src/commands/.spec/rules/ai/task-context/prep-optimization.md)).
