...
- **`<state>`** — one of:
  ...
  - `waiting` — the reviewer's invocation is in a rate-limit wait. A short transient-error backoff does not move a reviewer into `waiting`; it stays `running` (consistent with [src/ui/.spec/rules/waiting-footer-applies-to-long-waits-only.md](/src/ui/.spec/rules/waiting-footer-applies-to-long-waits-only.md)).
...
### Compaction
The reviewing footer line is compacted to fit the terminal width. The compaction decision is recomputed on every redraw against the current state and the current terminal width, never frozen at the width of the last state change (see [src/ui/.spec/rules/state-driven-redraw.md](/src/ui/.spec/rules/state-driven-redraw.md)). The tiers, applied in order, are:
