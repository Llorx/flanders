...
The footer line keeps the orange rendering defined in `Footer line — normal state`. While it shows the reviewing-state line defined in `Footer line — reviewing state`, the orange holds for the animated indicator, the `review: ` prefix, and every reviewer entry still in `running` or `waiting`, while a reviewer entry that has reached a verdict is recolored by that verdict — green for `pass`, red for `fail` — as defined there. The horizontal separators (both the one inside the bottom-fixed block and the ones framing each per-task completion snapshot) use the terminal default color.
