...
The bottom-fixed block is present from the very first moment the command starts running, with one exception: when `[plan]` is omitted and `plans/` holds more than one file, the command first shows the startup plan-selection prompt (see [.spec/contracts/cli-commands/implement/overview.md#invocation](/.spec/contracts/cli-commands/implement/overview.md#invocation) and [.spec/contracts/cli-commands/implement/non-interactive.md](/.spec/contracts/cli-commands/implement/non-interactive.md)), and the block is drawn as soon as a plan has been selected. That prompt is the only thing ever shown before the block. Aside from that prompt, the block appears before any other output is produced — including argv error messages, plan validation diagnostics, git preflight diagnostics, the tasks-completed noop message, and the streaming output of the very first task — and stays anchored at the bottom for the entire run. The single output channel that owns the block is created as the command's first action regardless, per [src/ui/.spec/rules/ui-behavior.md#single-output-channel-per-command-with-a-live-region](/src/ui/.spec/rules/ui-behavior.md#single-output-channel-per-command-with-a-live-region); only the drawing of the four pinned lines is deferred until a plan has been selected. Once drawn, the block is never removed from the screen: it remains visible after the command process has exited, and the user's shell prompt resumes on the line immediately below the block, not in place of it. If the command exits during the plan-selection prompt, before the block has been drawn, no block is shown. The only state change at exit is on the footer line, as defined in `Cleanup on exit`. Every other rule in this contract — output always above the block, the block always present, the block redrawn on every state change and on resize — applies from the moment the block is drawn and is never suspended thereafter.
...
During the adversarial review stage, the live time counter advances only while at least one reviewer is in its `running` state (see `Footer line — reviewing state`). When no reviewer is `running` — every reviewer is in `waiting` or has reached its `pass` or `fail` verdict — the counter pauses, and it resumes the moment any reviewer is `running` again. A paused interval is excluded from the active working time both the `task` and `plan` figures measure, so it never surfaces later in either figure or in the task's persisted time.
...
The working label and its animation are present from the moment the block is drawn and persist across every phase of the run — plan validation, git preflight, the build and test command detection that runs at startup (see [.spec/contracts/cli-commands/implement/workspace.md](/.spec/contracts/cli-commands/implement/workspace.md)), the tasks-completed noop case, and the iteration loop — until the command is about to exit, at which point the footer line transitions to the terminal label defined in `Cleanup on exit`. Because the block is drawn only after the startup plan-selection prompt when that prompt runs (see `Layout`), the working label first appears once a plan has been selected. One phase interrupts the working label: the adversarial review stage, when it shows the reviewing state defined in `Footer line — reviewing state`.

## Footer line — waiting state
While the runner is waiting for a retry, the footer line transitions to a label that conveys wait status to the user. A waiting session keeps retrying periodically behind the end the wait announces, so it resumes as soon as the limit actually clears even when the announced end is still hours or days away; the waiting state holds across a retry that finds the session still limited, and ends when a retry gets the session working again. The label therefore conveys both the wait's own end and the pending retry, and the user can bring that retry forward with F5 (see [.spec/contracts/cli-commands/implement/non-interactive.md](/.spec/contracts/cli-commands/implement/non-interactive.md), `Key input`). The exact label content, the wait information shown — including how the pending retry is presented — and which retries trigger this state are defined by rules.
...
- **`<state>`** — one of:
  ...
  - `waiting <countdown>` — the reviewer's invocation is in a rate-limit wait, followed by a compact live countdown of that reviewer's own remaining wait, and then by ` retry <retry-countdown>` while that reviewer's next retry falls before the end of its own wait. Each rate-limited reviewer shows its own two countdowns, recomputed independently of the others, because each entered its wait at its own moment and retries on its own schedule. A short transient-error backoff does not move a reviewer into `waiting`; it stays `running` (consistent with [src/ui/.spec/rules/ui-behavior.md#the-waiting-footer-state-appears-only-for-long-retry-waits](/src/ui/.spec/rules/ui-behavior.md#the-waiting-footer-state-appears-only-for-long-retry-waits)).
  - `pass` — the reviewer finished and recorded no violations.
  - `fail` — the reviewer finished and recorded one or more violations.

When at least one reviewer entry shows a `retry <retry-countdown>`, the line ends with the literal ` (F5)` after the last entry, telling the user that key brings every pending retry forward at once (see [.spec/contracts/cli-commands/implement/non-interactive.md](/.spec/contracts/cli-commands/implement/non-interactive.md), `Key input`).

The `<countdown>` shown after `waiting` is that reviewer's remaining wait in a compact duration form, with no zero-padding of the numeric components, switching format at the same boundaries as the worker waiting countdown (see [src/ui/.spec/rules/ui-behavior.md#the-waiting-footer-label-shows-a-heading-an-expected-end-a-countdown-and-the-pending-retry](/src/ui/.spec/rules/ui-behavior.md#the-waiting-footer-label-shows-a-heading-an-expected-end-a-countdown-and-the-pending-retry)):
...
The `<retry-countdown>` shown after `retry` is the time left until that reviewer's next retry, which is always shorter than one hour, so it takes the `<minutes>m` form of the same scheme (for example, `12m`).

Both countdowns recompute on every redraw from the current clock and, respectively, that reviewer's target end time and its next-retry instant, and are never stored as precomputed strings between redraws (see [src/ui/.spec/rules/ui-behavior.md#live-terminal-regions-redraw-from-structured-state](/src/ui/.spec/rules/ui-behavior.md#live-terminal-regions-redraw-from-structured-state)).

The animated indicator, the `review: ` prefix, and any reviewer entry still in its `running` or `waiting` state are rendered in the same orange as the rest of the footer line. Once a reviewer entry reaches a verdict, its whole entry — the `<tool>`, the `(<model> <effort>)` descriptor, and the `<state>` word together — is recolored by that verdict: green for `pass` and red for `fail`.
...
2. **Compact form** — when the full form does not fit, the `(<model> <effort>)` descriptor is dropped from every entry, leaving `<indicator> review: <tool>: <state>, …`.
3. **Truncation** — when the compact form also does not fit, the line is truncated with an ellipsis at the end.

A reviewer's `waiting <countdown>` keeps its countdown, its `retry <retry-countdown>` segment, and the line's trailing ` (F5)` hint in both the full and the compact form: the compact tier drops only the `(<model> <effort>)` descriptor, never the `<state>`, the countdowns that follow it, or the hint. Those are cut only when the line reaches the truncation tier. The leading animated indicator and the `review: ` prefix that follows it are kept in every tier — the compact tier drops only the per-entry `(<model> <effort>)` descriptor, and truncation removes characters from the end of the line, so the indicator and prefix at the start always remain.
...
The footer line keeps the orange rendering defined in `Footer line — normal state`. While it shows the reviewing-state line defined in `Footer line — reviewing state`, the orange holds for the animated indicator, the `review: ` prefix, every reviewer entry still in `running` or `waiting` — including a waiting entry's `retry <retry-countdown>` segment — and the line's trailing ` (F5)` hint, while a reviewer entry that has reached a verdict is recolored by that verdict — green for `pass`, red for `fail` — as defined there. The horizontal separators (both the one inside the bottom-fixed block and the ones framing each per-task completion snapshot) use the terminal default color.
...
- Interruption — the user interrupted the command (for example, with Ctrl+C). The label is one variant from the interruption pool.
