...
The skill artifacts `update` writes for a given tool and scope are identical to those a fresh `install` writes for that tool and scope. What is pinned is that after a successful `update` run, every AI-tool environment that already had Flanders skills can invoke `/flanders-spec`, `/flanders-plan`, and `/flanders-work` at their current version.
