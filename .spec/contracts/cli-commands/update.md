...
## Purpose
Refresh the Flanders skills already delivered to the user's AI-tool environment(s), rewriting each installed skill artifact with the current version's content so that an upgraded Flanders package republishes its `/flanders-spec`, `/flanders-plan`, `/flanders-implement`, and `/flanders-hard-stop-review` skills without the user re-answering any configuration question. The command republishes skills only; it leaves the persistent `.flanders/` configuration exactly as it was.
