...
- `--skills-tool=<value>` — which AI tool(s) the skills are installed for. The value is a comma-separated list of one or more distinct names drawn from `claude` and `codex` (for example `claude`, `codex`, or `claude,codex`). The skills are installed for every tool the list names.
...
- `--reviewer-minimum=<value>` — the minimum number of reviewers that must run to a verdict in each review round (see [.spec/contracts/shared/task-cycle.md](/.spec/contracts/shared/task-cycle.md)). The value must be an integer between `1` and the number of configured reviewers, inclusive.

Supplying any weighted-review flag (`--reviewer-minimum` or any `--reviewer[-N]-optional`) with a single-reviewer configuration is a usage error. Supplying `--reviewer-minimum` equal to the number of configured reviewers together with any `--reviewer[-N]-optional` flag is also a usage error: a minimum equal to the reviewer count forces every reviewer to run to a verdict, so no reviewer can be optional.

Any tool, model, effort, or fast question whose answer was not supplied via flags is prompted interactively (see `Interactive prompts`). Any question whose answer was supplied via a flag is not prompted again. The questions whose values form a closed set — every tool question — reject a flag value outside that set as a usage error, and `--skills-tool` accepts only a comma-separated list of distinct names drawn from the closed tool set. The questions whose values are open — every model question and every effort question — accept any flag value verbatim and are never rejected on value-set grounds. The fast question is answered by a presence flag (`--worker-fast`, `--reviewer-fast`, `--reviewer-N-fast`) rather than a value: the flag enables fast mode for its role, and it is rejected as a usage error only when that role's tool is not `claude` or its model does not support fast mode.
...
A model identifier is always an open value: whatever the user selects — from a top-level list or from a drill-down submenu — types into the `codex` free-text fallback, or types into the `claude` custom entry is persisted verbatim. The `--worker-model`, `--reviewer-model`, and `--reviewer-N-model` flag equivalents follow the same rule: any value is accepted verbatim, and an empty value or an omitted flag answered as empty resolves to "default configured model".

### Effort selection
For each tool selected as the worker or as any reviewer, the effort question is rendered according to the tool:
- For `codex`, the effort levels are sourced from the same account query that supplies the `codex` model list (see `Model selection`): the list is the reasoning-effort levels the tool reports for the model selected in the preceding model question, plus one entry, `default configured effort`, that resolves to "do not pass an explicit effort and let the tool use its default". When the preceding model question did not resolve to a specific reported model — the `default configured model` entry was chosen, a model was typed into the free-text fallback, or `codex` could not be contacted — the effort question falls back to a free-text input with the placeholder `leave empty for the default configured effort`, whose empty answer resolves to the same "default configured effort" semantics. A `codex` effort value is open: whatever the user selects from the list or types is persisted verbatim.
...
Accepting every pre-selected default — pressing through the configuration-derived questions without changing any answer — reproduces the stored configuration exactly, so a re-run at the same scope that changes nothing writes back the same `.flanders/config.json`. A question pre-selects a stored answer only when that answer is still among the options the question currently offers. Every question other than the probe-sourced `codex` model and `codex` effort questions can always offer its stored value: the `claude` model question and the `claude` effort question through their custom entry, the tool questions because a stored value there is always a member of their fixed set, and the minimum through its free-text entry. The probe-sourced `codex` questions offer only what the account query currently returns: the `codex` model question offers the models the probe returns plus `default configured model`, and the `codex` effort question, when it renders as a selectable list for a chosen model, offers that model's reported effort levels plus `default configured effort`. A stored `codex` model the probe no longer returns, or a stored `codex` effort the chosen model no longer reports, is not among the options, so that question is presented without a pre-selected default and is answered actively; when the `codex` effort question falls back to free-text, its stored value seeds that input.

## Skills produced
For each AI tool the user picked for skills, the command writes one skill artifact per Flanders skill (`/flanders-spec`, `/flanders-plan`, `/flanders-implement`, `/flanders-hard-stop-review`) into that tool's skill folder for the selected scope:
...
- A single-valued tool flag (`--worker-tool`, `--reviewer-tool`, or `--reviewer-N-tool`) is supplied with a value outside its closed set: exits non-zero with a diagnostic that names the offending flag and value. Model flags and effort flags are open and never trigger this error.
- `--skills-tool` is supplied with a value that is not a comma-separated list of one or more distinct names drawn from `claude` and `codex` (an empty list, an unknown name, or a repeated name): exits non-zero with a diagnostic that names the offending value.
...
## Out of scope
- The exact internal contents of each skill artifact (frontmatter fields, body shape) are implementation choices and are not pinned by this contract. What is pinned is that after a successful `install` run, the user is able to invoke `/flanders-spec`, `/flanders-plan`, `/flanders-implement`, and `/flanders-hard-stop-review` from inside an AI-tool session of each selected tool whose skills root is the chosen scope.
