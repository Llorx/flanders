...
The defaults reproduce a run where no reviewer is ever cancelled: the minimum equal to the number of reviewers and every reviewer required. The minimum and the per-reviewer optional flags follow the same flag-versus-prompt behavior as every other answer, pinned in [src/commands/.spec/rules/install/weighted-reviews-configuration.md](/src/commands/.spec/rules/install/weighted-reviews-configuration.md). When the reviewer list holds a single reviewer, this section is not shown: that reviewer is required and the minimum is `1`.
