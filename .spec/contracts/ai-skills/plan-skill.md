...
- A task that constrains a mechanism — reusing existing code unchanged, leaving a function or file untouched, or declaring code out of the task's scope — is achievable against the baseline it leaves immutable: every contract or rule the task's acceptance criteria require the task's output to satisfy is already satisfied by that immutable baseline, so honoring the no-touch constraint still leaves every acceptance criterion reachable. When the immutable baseline carries a gap against such a required obligation that the no-touch constraint would prevent the task from closing, the acceptance criteria are unreachable and the task is invalid even when every claim it makes about the baseline is accurate; the planner instead widens the task's scope to authorize and ground the change to that baseline, or escalates the conflict to the user during the clarification phase.
