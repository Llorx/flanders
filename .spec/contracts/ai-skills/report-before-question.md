...
The end-of-run next-step launch question — the question `/flanders-spec`, `/flanders-hard-stop-review`, and `/flanders-implement` put to the user after their final report — is asked as plain chat text at the end of the same chat message that carries that final report, so the report and its question arrive together in one message.
