...
- A **rule** captures a single, atomic piece of implementation guidance internal to the directory the `.spec` scopes — a constraint, convention, or pattern that the directory's code must follow. The rule is the atomic unit, not the file: each rule pins one obligation and carries its own scope of enforcement. A rule file holds either one rule on its own or several related rules as discrete atomic sections, and a scope's rules are organized in whichever of these shapes keeps each rule atomic and each file's subject coherent: a standalone file for an isolated rule, one file grouping a related cluster as sections, or a subfolder of files when the scope spans several distinct clusters.
