...
2. **Worker stage.** Spawn a worker AI whose tool, model, reasoning effort, and fast-mode setting are the worker fields of the Flanders configuration (see [.spec/contracts/shared/flanders-config.md](/.spec/contracts/shared/flanders-config.md)).
   ...
   - Instructions to honor the obligations of every contract, rule, and behavior rule that applies and implement the task. Those obligations are the whole of the code practice the worker is held to; the prompt adds a standard of its own only for the source comments the worker writes.
   - The full list of contract files, the full list of rule files, and the full list of behavior-rule files, each file given by its project-root-relative namespace followed by that file's headings (see [.spec/contracts/shared/spec-section-model.md#heading-listing](/.spec/contracts/shared/spec-section-model.md#heading-listing)), so the worker reads what each file carries without opening it. The worker may consult any file in these lists at its discretion. A surface that also consolidates reference content for the worker to read defines that consolidation itself.
   ...
   When the worker invocation completes, the surface first checks for a `hard-stop.log` file in the temporary folder: when it exists, the run hard-stops directly per the Hard stop section below, and this iteration's build, test, and review stages do not run. Otherwise the surface runs the applicable-spec sweep below and then, before the build gate runs, stages every change in the working tree with `git add -A`, so the worker's output — including any files it created that were never tracked — is in the index for the gates that follow and for the eventual commit.

   **Applicable-spec sweep.** Which files a worker's work touches is settled by doing the work, so the obligations its change set triggers are known only once it has finished. The surface establishes them itself: it derives the change set's paths from git — every path the working tree reports as modified, created, deleted, or renamed, reduced against the run baseline the same way each reviewer's change set is — and obtains the specs governing those paths per [.spec/contracts/cli-commands/specs.md](/.spec/contracts/cli-commands/specs.md). A spec that answer names counts as already given to the worker when the surface consolidated its content for this cycle's worker or named it in an earlier sweep of this cycle; every other spec the answer names is a new spec.

   When the sweep finds new specs, the surface continues the worker's own session with a short addendum stating that the changes it produced bring further obligations into scope, naming each new spec file by its path together with the titles of that file's sections, and directing the worker to establish which of them its work triggers and to satisfy those it does. The addendum carries the paths and section titles alone, so the worker opens whichever file it needs. When that turn completes, the surface checks for `hard-stop.log` exactly as above, then sweeps again against the change set as it now stands. The sweep repeats until it finds no new spec, a point it reaches because the set of specs already given only grows while the specs a project holds are finite.

   These continuation turns belong to the worker stage: they leave `iteration` unchanged and consume none of the iteration budget. The sweep runs while the worker's session can be continued; where it cannot (see [src/commands/.spec/rules/ai/task-context.md#when-resume-is-not-available](/src/commands/.spec/rules/ai/task-context.md#when-resume-is-not-available)), the surface proceeds to the build gate with the change set as it stands.
