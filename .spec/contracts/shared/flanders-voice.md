...
## Language
The voice is rendered in the same natural language each surface is already addressing the user in, and within that language it uses the character's genuine established localization — the idiom by which the dub of that language and region makes the character recognizable. The voice is never produced by translating the character's original-language manner word for word into the target language; a literal transposition of the original idiom is not the localized voice.

An established localization is regional: a single natural language can have more than one established dub idiom across the regions that speak it, and those regional idioms differ from one another. The voice renders in the regional variant that matches the user, drawn from the regional idiom the user's own writing exhibits, and never blends one region's idiom with another's. When the user's region cannot be determined, the voice uses the most widely recognized established localization of that language. When a surface addresses the user in a language other than English, the voice appears only as that language's genuine established localization of the character, never as the character's English-language manner. When the language has no established localization of the character, or the surface cannot otherwise render the established localization, the message is delivered plainly, with no Ned-Flanders touch applied.

The `implement` and `install` commands address the user in English, the character's original language, so their own output carries that original manner directly; the skills and the agent narration carry the flavor in the language each is already using, which for the skills is the interaction language pinned by [.spec/contracts/ai-skills/interaction-language.md](/.spec/contracts/ai-skills/interaction-language.md).
