...
## Purpose
Define the persistent Flanders configuration surface — the `.flanders/` folder — that the `install` command writes (see [.spec/contracts/cli-commands/install.md](/.spec/contracts/cli-commands/install.md)), that other Flanders surfaces (currently the `implement` command and the `/flanders-implement` skill) read to run, and that `install` itself reads at the chosen scope to pre-select its interactive defaults.
...
## Fields persisted
The folder persists the answers the user supplied to the `install` command that downstream Flanders surfaces need at run time:
- The AI tool the worker of the single-task cycle uses — the `implement` command's worker and the `/flanders-implement` skill's worker alike (see [.spec/contracts/shared/task-cycle.md](/.spec/contracts/shared/task-cycle.md)): one of `claude` or `codex`.
...
- The ordered list of adversarial reviewers every review round of that cycle runs. The list holds one or more reviewers, in the order the user configured them, and each reviewer carries its own five fields:
  ...
  - Whether the reviewer is optional: a reviewer marked optional may be cancelled before it finishes once its review round can complete without it, while a reviewer not marked optional (required) always runs to a verdict and is never cancelled. See [.spec/contracts/shared/task-cycle.md](/.spec/contracts/shared/task-cycle.md).
- The minimum number of reviewers that must run to a verdict in each review round. It is at least `1` and at most the number of configured reviewers, and it is `1` when the reviewer list holds a single reviewer. When it equals the number of configured reviewers, every reviewer is required, because that leaves no reviewer that can be cancelled.
