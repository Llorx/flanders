...
**Complete coverage.** The `README.md` documents the whole usable surface a user can invoke: every CLI command together with the arguments, flags, and behavior each one carries; every AI-tool skill together with what invoking it does; the requirements for running the library; how the library is installed; how it is configured; and how it is used from start to finish. Each part of the usable surface a user can operate is present in the `README.md`. The conceptual orientation a reader needs to understand what the library is and how its surfaces fit together — the spec → plan → implement cycle — is part of that usable surface and is documented too. That orientation includes the scope of what Flanders enforces: the `README.md` states that the practices Flanders holds the project's design, architecture, and code quality to are the ones the user writes into the spec corpus — so whether SOLID, a duplication policy, a style guide, or any other practice governs the project is the user's own decision, and Flanders brings none of its own — and it states the one discipline Flanders does apply of its own initiative regardless of what the corpus carries, the source-comment discipline pinned by [src/prompts/.spec/rules/ai/code-comment-economy.md](/src/prompts/.spec/rules/ai/code-comment-economy.md). Coverage is a matter of presence: each part of the usable surface appears, and the depth its description reaches is governed by [.spec/rules/readme-presentation.md](/.spec/rules/readme-presentation.md).
...
- This rule governs which topics the `README.md` covers and which it leaves out. How it presents them — the depth each description reaches, where each topic is explained, and the shape of its steps and sections — is governed by [.spec/rules/readme-presentation.md](/.spec/rules/readme-presentation.md), and the tone the `README.md` is written in by the Flanders voice (see [.spec/contracts/shared/flanders-voice.md](/.spec/contracts/shared/flanders-voice.md)).
