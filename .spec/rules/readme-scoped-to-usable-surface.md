...
**Complete coverage.** The `README.md` documents the whole usable surface a user can invoke: every CLI command together with the arguments, flags, and behavior each one carries; every AI-tool skill together with what invoking it does; the requirements for running the library; how the library is installed; how it is configured; and how it is used from start to finish. Each part of the usable surface a user can operate is present in the `README.md`. The conceptual orientation a reader needs to understand what the library is and how its surfaces fit together — the spec → plan → implement cycle — is part of that usable surface and is documented too. That orientation includes the scope of what Flanders enforces: the `README.md` states that the practices Flanders holds the project's design, architecture, and code quality to are the ones the user writes into the spec corpus — so whether SOLID, a duplication policy, a style guide, or any other practice governs the project is the user's own decision, and Flanders brings none of its own — and it states the disciplines Flanders does apply of its own initiative regardless of what the corpus carries: the source-comment discipline pinned by [src/prompts/.spec/rules/ai/code-comment-economy.md](/src/prompts/.spec/rules/ai/code-comment-economy.md), and the instruction to cover new behavior with tests that the worker of the single-task cycle carries, pinned by [.spec/contracts/shared/task-cycle.md#the-cycle](/.spec/contracts/shared/task-cycle.md#the-cycle).
